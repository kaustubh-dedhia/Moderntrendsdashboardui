...
import { Edit2, ChevronDown, AlertCircle, Upload, X, ArrowLeft } from 'lucide-react';
...
export default function EditProduct() {
  ...
  useEffect(() => {
    if (product) {
      ...
      setProductImage(product.initialImage || null);
    }
  }, [product]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const existingProductCode = products.find((p) => p.productCode === productCode && p.id !== id);
    if (existingProductCode) {
      setError('Product code already exists. Please use a different code.');
      return;
    }

    // Check for duplicate product name within the same brand (excluding current product)
    ...
    return (
      <div className="max-w-3xl mx-auto">
        <div className="bg-white/90 backdrop-blur-sm rounded-3xl p-16 text-center border border-[#bab0aa]/20 shadow-xl animate-scaleIn">
          <p className="text-[#745b4e] text-lg mb-8 font-medium">Product not found</p>
          <button
  ...
  return (
    <div className="max-w-3xl mx-auto">
      ...
      <div className="mb-8 animate-slideInUp">
        <h1 className="text-4xl font-bold bg-gradient-to-r from-[#745b4e] via-[#215b2f] to-[#745b4e] bg-clip-text text-transparent mb-3">Edit Product</h1>
        <p className="text-[#bab0aa] text-lg">Update product information</p>
      </div>

      <form onSubmit={handleSubmit} className="bg-white/90 backdrop-blur-sm rounded-2xl p-8 border border-[#bab0aa]/20 shadow-lg animate-fadeIn">
        <div className="space-y-6">
          ...
          <div className="pt-4 flex gap-3">
            <button
              type="submit"
              className="flex-1 flex items-center justify-center gap-2 py-3.5 px-6 rounded-xl bg-gradient-to-r from-[#215b2f] to-[#1a4825] text-white font-semibold hover:shadow-lg hover:shadow-[#215b2f]/30 hover:scale-[1.02] transition-all duration-200"
            >
            ...
            <button
              type="button"
              onClick={() => navigate('/products')}
              className="px-8 py-3.5 rounded-xl bg-[#bab0aa]/20 text-[#745b4e] font-semibold hover:bg-[#bab0aa]/30 transition-all duration-200"
            >
              Cancel
            </button>
          </div>
